...
% % % % Update History % % % %
% 
% gamma - changing synaptic conductance
% no longer does a neuron have an axonal synaptic conductance,
% a neuron has a synaptic conductance inside itself
% actually i decided that's dumb and doesnt need to be the way it is
...
n1 = 10;   % number of neurons in the first population
...
for j = 1:tnum-1
    ...
    % calculate intrinsic frequency
    Isummed = (I + delta*s(j,:));
    %Isummed = I + (delta*y(j,:)) * A * s(j,:); 
    ...
    s(j+1,:)     = s(j,:)   + dt * siODE(s(j,:),taus);
    
    e=1; ss=0;
    while any(e)
        % check if any neuron is above pi ( should be pmax )
        e = (theta(j+1,:)>pi); a=find(e); anot = ~e;
        ...
        %update downstream nuerons synaptic current
        % reassign connectivity matrix
        Astar = A;
        % set non-firing neurons to zero output
        Astar(anot,:) = 0;
        % addjust firing neurons output by their synaptic depression
        Astar = Astar.*y(j,:)';
        
        s(j+1,:) = s(j+1,:) + sgain*(1-s(j+1,a));
