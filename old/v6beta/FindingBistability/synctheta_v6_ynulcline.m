function [sps,yss,yss_n2,yssmin,yssmax] = synctheta_v6_ynulcline(params,D)
%
...
yss = mean(mean(y(end/2:end,:)));
yss_n2 = mean(mean(y(end/2:end,n1+1:n)));

yssmin = min(mean(y(end/2:end,:)));
yssmax = max(mean(y(end/2:end,:)));
