...
% % % % Update History % % % %
%
% Version 6 beta - have to include ydrop ( synaptic depression ). calculate
% it at steady state, then use it to set the synaptic strength of 
% each neuron for voltage nulc lines
%
...
% check for update 6 beta that sim is long enough
if tmax < tauy*4 %(5 seconds)
    error('pls make sim time longer, y takes a long time to reach ss')
end
time2settle = tauy*5; % time we think system takes to get to steady state
...
for j = 1:tnum-1
    ...
    % set connectivity to matrix to synaptic strength as determined by the
    % yss value
    if t(j) == time2settle
        % calculate yss based on last second of data
        y_acountstatic = mean(y(j-1000/dt:j,:));
    end
    ...
    if t(j) < time2settle
        ...
        Isummed = I + (delta*y_acountstatic.*si(j,:)) * A; 
    end
